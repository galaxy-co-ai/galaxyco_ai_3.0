...
// App ticker with sidebar awareness
export function StockTicker() {
  ...
  return (
    <div className={`fixed bottom-0 left-0 right-0 z-40 pb-4 px-4 pointer-events-none transition-all duration-300 ${state === "expanded" ? "md:left-[13rem]" : "md:left-[3rem]"}`}>
      <div className="relative overflow-hidden bg-gradient-to-r from-muted/30 via-muted/50 to-muted/30 rounded-xl py-2 shadow-lg backdrop-blur-sm pointer-events-auto">
        <div className="animate-ticker flex gap-2">
          {duplicatedItems.map((item, index) => (
            <Card
              key={`${item.id}-${index}`}
