...
export async function GET(
  ...
  try {
    ...
    const rateLimitResult = await rateLimit(`knowledge-share-list:${userId}`, 100, 3600);
...
export async function POST(
  ...
  try {
    ...
    // Rate limit - 100 requests per hour
    const rateLimitResult = await rateLimit(`knowledge-share-create:${user.id}`, 100, 3600);
    if (!rateLimitResult.success) {
      return NextResponse.json(
        { error: 'Rate limit exceeded. Please try again later.' },
        { status: 429, headers: {
...
export async function DELETE(
  ...
  try {
    ...
    // Rate limit - 100 requests per hour
    const rateLimitResult = await rateLimit(`knowledge-share-delete:${userId}`, 100, 3600);
    if (!rateLimitResult.success) {
      return NextResponse.json(
        { error: 'Rate limit exceeded. Please try again later.' },
        { status: 429, headers: {
