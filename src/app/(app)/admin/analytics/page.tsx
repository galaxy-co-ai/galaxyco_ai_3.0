...
import { PageViewTrendChart, UserActivityTrendChart } from '@/components/admin/AnalyticsCharts';
        ...
        <Card>
          ...
          <CardContent>
            {pageViewTrend.length > 0 ? (
              <PageViewTrendChart data={pageViewTrend.map(d => ({ date: d.date, value: d.views }))} />
            ) : (
        ...
        <Card>
          ...
          <CardContent>
            {userActivityTrend.length > 0 ? (
              <UserActivityTrendChart data={userActivityTrend.map(d => ({ date: d.date, value: d.users }))} />
            ) : (
              <p className="text-muted-foreground text-center py-8 text-sm">No data yet</p>
            )}
            <div className="mt-2 text-xs text-muted-foreground flex items-center justify-between">
