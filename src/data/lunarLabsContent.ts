...
export interface PathStep {
  ...
  prerequisites?: string[]; // Array of topic IDs that must be completed first
}
...
export interface LearningPath {
  ...
  description: string;
  icon: string;
  estimatedTime: string; // Total time for entire path
  difficulty: 'beginner' | 'intermediate' | 'advanced';
  steps: PathStep[];
  milestones: {
    percentage: number;
...
export const learningPaths: LearningPath[] = [
  {
    ...
    description: 'Master CRM, lead scoring, and sales automation to close deals faster',
    icon: '💼',
    estimatedTime: '45 min',
    difficulty: 'intermediate',
    steps: [
      { topicId: 'getting-started', order: 1, required: true },
      { topicId: 'crm-deep-dive', order: 2, required: true, prerequisites: ['getting-started'] },
      { topicId: 'integrations-gmail', order: 3, required: true, prerequisites: ['crm-deep-dive'] },
      { topicId: 'workflows-basics', order: 4, required: true, prerequisites: ['integrations-gmail'] },
      { topicId: 'ai-agents-intro', order: 5, required: false, prerequisites: ['workflows-basics'] }
    ],
    milestones: [
      { percentage: 25, title: 'Getting Started', description: 'Completed onboarding', achievementId: 'first-demo' },
      { percentage: 50, title: 'CRM Expert', description: 'Mastered CRM features', achievementId: 'crm-expert' },
      { percentage: 75, title: 'Automation Pro', description: 'Set up workflows and integrations' },
      { percentage: 100, title: 'Sales Master', description: 'Completed Sales Mastery Path', achievementId: 'ai-commander' }
    ]
  },
  {
    id: 'support-path',
    role: 'support',
    title: 'Support Excellence Path',
    description: 'Learn to resolve tickets faster with AI-powered knowledge base and automation',
    icon: '🎧',
    estimatedTime: '35 min',
    difficulty: 'beginner',
    steps: [
      { topicId: 'getting-started', order: 1, required: true },
      { topicId: 'knowledge-base', order: 2, required: true, prerequisites: ['getting-started'] },
      { topicId: 'integrations-gmail', order: 3, required: true, prerequisites: ['knowledge-base'] },
      { topicId: 'ai-agents-intro', order: 4, required: false, prerequisites: ['integrations-gmail'] }
    ],
    milestones: [
      { percentage: 33, title: 'Knowledge Base Setup', description: 'Configured knowledge base' },
      { percentage: 66, title: 'Integration Master', description: 'Connected email integration', achievementId: 'integration-master' },
      { percentage: 100, title: 'Support Hero', description: 'Completed Support Excellence Path' }
    ]
  },
  {
    id: 'marketing-path',
    role: 'marketing',
    title: 'Marketing Growth Path',
    description: 'Automate campaigns, score leads, and drive growth with AI',
    icon: '📈',
    ...
    steps: [
      { topicId: 'getting-started', order: 1, required: true },
      { topicId: 'crm-deep-dive', order: 2, required: true, prerequisites: ['getting-started'] },
      { topicId: 'workflows-basics', order: 3, required: true, prerequisites: ['crm-deep-dive'] },
      { topicId: 'ai-agents-intro', order: 4, required: false, prerequisites: ['workflows-basics'] }
    ],
    milestones: [
      { percentage: 25, title: 'CRM Basics', description: 'Learned CRM fundamentals' },
      { percentage: 50, title: 'Workflow Wizard', description: 'Created automated workflows', achievementId: 'workflow-wizard' },
      { percentage: 100, title: 'Marketing Maestro', description: 'Completed Marketing Growth Path' }
    ]
  },
  {
    id: 'operations-path',
    role: 'operations',
    title: 'Operations Efficiency Path',
    description: 'Streamline processes and automate workflows for maximum efficiency',
    icon: '⚙️',
    estimatedTime: '50 min',
    difficulty: 'intermediate',
    steps: [
      { topicId: 'getting-started', order: 1, required: true },
      { topicId: 'workflows-basics', order: 2, required: true, prerequisites: ['getting-started'] },
      { topicId: 'ai-agents-intro', order: 3, required: true, prerequisites: ['workflows-basics'] },
      { topicId: 'knowledge-base', order: 4, required: false, prerequisites: ['ai-agents-intro'] }
    ],
    milestones: [
      { percentage: 33, title: 'Workflow Builder', description: 'Created first workflow', achievementId: 'workflow-wizard' },
      { percentage: 66, title: 'AI Commander', description: 'Deployed AI agents', achievementId: 'ai-commander' },
      { percentage: 100, title: 'Operations Expert', description: 'Completed Operations Efficiency Path' }
    ]
  },
  {
    id: 'developer-path',
    ...
    description: 'Build custom workflows and integrate with any API',
    icon: '👨‍💻',
    ...
    milestones: [
      { percentage: 50, title: 'Workflow Master', description: 'Mastered workflow building', achievementId: 'workflow-wizard' },
      { percentage: 100, title: 'Integration Expert', description: 'Completed Developer Integration Path', achievementId: 'ai-commander' }
    ]
  }
];
...
export const rolePersonalization = {
  ...
  operations: {
    ...
    learningPathId: 'operations-path'
  },
  developer: {
