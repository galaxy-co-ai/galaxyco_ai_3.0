...
export function TopicExplorer({ selectedTopicId, onSelectTopic, roleFilter }: TopicExplorerProps) {
  ...
  return (
    <Card className="p-2.5 border-purple-500/20 flex flex-col overflow-hidden" style={{ maxHeight: 'calc(100vh - 420px)' }}>
      <div className="flex items-center gap-2 mb-2 flex-shrink-0">
        <span className="text-base">📚</span>
        <h3 className="text-xs">Topics</h3>
      </div>
          ...
          {Object.entries(categories).map(([category, categoryTopics]) => (
            <div key={category}>
              <button
                ...
                className="w-full flex items-center justify-between p-1.5 hover:bg-gray-800/50 rounded transition-colors"
              >
